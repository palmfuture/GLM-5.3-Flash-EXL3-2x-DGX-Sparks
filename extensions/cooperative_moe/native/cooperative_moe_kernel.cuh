...
// GLM-5.3-Flash TP2 compile-time specialization of the upstream two-stage
...
//
// H=4096, packed moe_intermediate_size=2048, TP2 column/row shard → local I=1024
// (overlay/exl3.py _exl3_intermediate_local; tests/bench_e3_microbench.py).
// Top-k 8 (EXL3_FAT_GROUPED_TOPK). DFlash2 k=7 → 8 rows/seq; MAX_NUM_SEQS=4 → 32.
...
__device__ __forceinline__ MoeCoopParams glm53_fixed_params(const MoeCoopParams& input)
{
    ...
    p.topk = 8;
    p.slots_max = 256; p.rows_max = 32;
    p.ctr_a_len = 2048; p.ctr_b_len = 1024;
...
static_assert(MAX_SLOTS == 256, "256-slot ceiling; rank key (expert<<8)|slot");
static_assert(MAX_SLOTS <= 256, "run-builder packs slot into 8 bits");
