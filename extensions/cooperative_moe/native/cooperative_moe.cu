// Isolated GLM-5.3-Flash TP2 specialization of the upstream two-stage MoE kernel
...
namespace ns = glm53_coop_ns;
constexpr int H = 4096, I = 1024, TOPK = 8, ROWS_MAX = 32;
constexpr int SLOTS_MAX = TOPK * ROWS_MAX;
constexpr int CB_MCG = 1;
static bool prepared[3] = {};
// Run builder packs slot into 8 bits: key = (expert << 8) | slot. Raising
// SLOTS_MAX past 256 requires a representation change, not just more scratch.
static_assert(SLOTS_MAX == TOPK * ROWS_MAX, "slot capacity is rows * top-k");
static_assert(SLOTS_MAX <= 256, "run-builder rank key packs slot in 8 bits");
...
static_assert(CTR_LEN == 3587, "counter/run-table allocation");

struct Selected { void* a; void* b; int sa; int sb; bool wa; bool wb; };

Selected select(int geometry) {
...
extern "C" int glm53_coop_abi() { return 1; }
extern "C" int glm53_coop_info(int bits, int geometry, int* info) {
...
// Buffers have capacity 256 routed slots and 32 output rows, prepared before capture.
extern "C" int glm53_coop_launch(void** t, int bits, int rows, int experts,
    float limit, int geometry, int gu_f32, void* stream_ptr) {
    if (!t || bits != 4 || rows < 1 || rows > ROWS_MAX || experts < 1 || experts > 288 ||
