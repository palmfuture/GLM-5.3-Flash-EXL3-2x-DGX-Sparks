...
class ProfileTests(unittest.TestCase):
    ...
    def test_unvalidated_pins_are_rejected(self):
        with patch.multiple(
            prepare_profile,
            STOCK_SHA=hashlib.sha256(self.stock.read_bytes()).hexdigest(),
            BINARY_SHA="UNVALIDATED-rebuild-required",
            ADAPTER_SHA=hashlib.sha256(
                (self.artifacts / "runtime.py").read_bytes()
            ).hexdigest(),
        ):
            with self.assertRaises(ValueError):
                self.generate()
