...
namespace GLM53_COOP_NAMESPACE {

// GLM-5.3-Flash TP3 compile-time specialization of the upstream two-stage
// cooperative MoE kernel (exllamav3 58d4d732). Device arithmetic is unchanged.
// Runtime row count, expert count/routing and activation limit remain dynamic.
// Host rejects unsupported geometry/shape or FP32 gate/up scratch.
//
// H=4096, EP partitions 288 global experts into 96 local full-I=2048 experts.
// Two TUs specialize physical row capacities 32 and 64; routing stays dynamic.
// K4 MCG is codebook cb=1 (exl3_moe_coop_prepare: mcg?1:mul1?2:0).
__device__ __forceinline__ MoeCoopParams glm53_fixed_params(const MoeCoopParams& input)
{
    MoeCoopParams p = input;
    p.H = p.Hi = p.Ho = p.H_out = 4096;
    p.I = 2048;
    p.x_stride = p.out_stride = 4096;
    p.topk = 8;
    p.slots_max = GLM53_ROWS_MAX * 8; p.rows_max = GLM53_ROWS_MAX;
    p.ctr_a_len = p.slots_max * 16; p.ctr_b_len = p.rows_max * 32;
...
constexpr int MAX_SLOTS = GLM53_ROWS_MAX * 8;
constexpr int SLOT_BITS = 9;
static_assert(GLM53_ROWS_MAX == 32 || GLM53_ROWS_MAX == 64, "row capacity");
static_assert(MAX_SLOTS <= (1 << SLOT_BITS), "packed ordering key capacity");
static_assert(MAX_SLOTS <= THREADS, "run builder needs one thread per slot");
static_assert(ROWS == 8, "run builder groups at most eight slots per expert");
...
)
{
    ...
    if (my_e >= 0)
    {
        const int key = (my_e << SLOT_BITS) | t;
        int rank = 0;
        for (int u = 0; u < slots; ++u)
        {
            const int eu = sh_local[u];
            rank += (eu >= 0 && ((eu << SLOT_BITS) | u) < key) ? 1 : 0;
        }
...
{
    const MoeCoopParams p = glm53_fixed_params(p_in);
